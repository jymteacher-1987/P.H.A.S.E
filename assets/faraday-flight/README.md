...
5스테이지 비행 슈팅. 코일·자석·축전기를 수집하며 전자기 유도의 작은 원리를 만납니다. 방향키/WASD 또는 상대 위치 드래그로 움직이고 자동으로 공격합니다. 스페이스나 오른쪽 아래 버튼으로 스파크 폭풍을 사용합니다. Esc는 일시정지입니다. 모든 진행 기록은 현재 기기의 localStorage에만 저장합니다. 외부 순위 서버나 분석 전송은 없습니다.

## 미술과 소리

- `hero.webp`: 패러데이와 가상 비행 장치, 8개 비행 동작.
...
`physics.js`의 유도 전압은 이상적인 한 코일의 모형 `emf = N B A ω sin(θ)`이며, 자기선속은 `B A cos(θ)`입니다. 면적과 자석의 배치를 고정하고 감은 수 N, 자기장 세기 B, 회전 속도 ω를 비교합니다. 정지한 자석은 지속적으로 전기를 생산하지 않습니다. 모든 자석은 N/S 한 쌍으로 표시합니다. 운동을 유지하는 바람과 엔진의 일이 전기 에너지의 원천입니다.
...
- 자석: 동일한 회전 조건에서 더 큰 자기장 변화와 유도 전압을 표현합니다.
- 과열 파편: 일부 자석이 높은 온도에서 약해질 수 있다는 성질을 소재로 삼았습니다. 7초의 회복이나 냉각 즉시 복구는 게임 규칙이며, 자석의 실제 약화에 관한 온도·영구성·회복을 시뮬레이션하지 않습니다.
- 축전기: 발전기로 충전하거나 충전된 아이템을 수집합니다. 저장 한도, 충전율, 탄막을 지우는 효과는 게임 규칙입니다. 충전 게이지는 정류된 유도 전압을 이용한 임의의 진행 단위이며 실제 J, V, C 측정값이 아닙니다. 내부 저항·회로 손실·정류 회로·부하를 풀지 않습니다.
- 전지는 오버드라이브를 위한 수집물입니다. 구리 부품까지 끌어오는 수집 효과는 별도의 부품 회수 장치이며, 모든 물체가 강하게 자석에 붙는다는 표현이 아닙니다.
