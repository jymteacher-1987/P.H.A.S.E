...
`weapons.js`는 코일·자석 수집과 구리 코일/강한 자석 업그레이드의 합으로 무기를 결정합니다. 진화 수치 0/3/9/17/26/32에서 스파크 미사일 → 트윈 미사일 → 펄스 레이저 → 쌍열 레이저 → 플라스마 캐논 → 오로라 캐논으로 변합니다. 기존 코일 8·자석 7 제한을 제거했으며, 마지막 형태 이후에도 피해와 출력 등급이 오르고 광선 폭·나선 효과가 성장합니다. 수집 수치는 저장·이어하기·재도전에 유지됩니다. 높은 수집량에서는 증가 폭을 완만하게 해 특수 공격 충전이나 피해가 폭주하지 않게 합니다. 저장 데이터는 정수 0~999 범위로 방어적으로 읽습니다.

레이저는 발사 순간 기체 앞에서 위로 곧게 뻗는 펄스입니다. 적을 자동 추적하지 않으며, 발사 뒤 기체가 움직여도 이미 발사한 광선 축은 고정됩니다. 한 펄스는 같은 적에게 한 번만 피해를 주고 잔광 렌더링은 피해를 추가하지 않습니다. 형태별 2~4기까지 관통하되 보스와 벽에서 멈추고, 진입 사격 전 적은 보호됩니다. 굵은 캐논은 탄 수를 늘리는 대신 광선의 폭·피해·나선 효과·발사음을 바꿉니다. 적 탄환은 지우지 않으며 방어는 여전히 이동과 스파크 폭풍으로 합니다.
