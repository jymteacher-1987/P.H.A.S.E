...
상단 정보는 두 줄로 줄였으며 보스전에는 보스 이름과 체력만 표시합니다. 공략 문구, 빈틈 표시, 장갑 상태 글자는 전투 화면에서 제거했습니다. 발전기 그림은 하단 중앙입니다. 보스 체력은 520/1,200/2,450/3,250/4,150이며 후반 단계와 낮아진 체력 구간에 따라 여러 겹의 탄막이 늘어납니다. 기본 발사 위치는 모두 보스 몸체 안의 발사구입니다. 분열탄은 몸체에서 출발한 뒤 이동 중에 갈라집니다. 보스 크기는 기존보다 키우되 이동 범위를 제한해 화면 밖으로 잘리지 않게 했고, 단계별 탄막의 예고 시간은 유지합니다.
