...
- `node tools/generate-previews.cjs --only=afterlight`로 10번 간섭계의 실제 플레이 화면을 캡처합니다. 저장 기록을 주입하지 않고 촬영 설정의 버튼 조작으로 진행합니다. A·B에 50%씩 도착하는 빛길, 게임 제목과 조작부가 함께 보이는 4:3 화면입니다.
