(function () {
  ...
  el('logoutBtn').addEventListener('click', async () => {
    if (saving || (dirty && !confirm('저장하지 않은 변경 내용을 버리고 로그아웃할까요?'))) return;
  ...
  el('catalogReload').addEventListener('click', () => {
    if (dirty && !confirm('저장하지 않은 변경 내용을 버리고 다시 불러올까요?')) return;
    loadCatalog();
  });
  el('catalogItem').addEventListener('change', () => {
    if (dirty && !confirm('저장하지 않은 변경 내용을 버리고 다른 활동을 선택할까요?')) {
