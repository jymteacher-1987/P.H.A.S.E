...
for(const name of ['game.js','physics.js','levels.js','style.css']){
...
const version=hash(['plays/afterlight.html','assets/afterlight/game.js','assets/afterlight/physics.js','assets/afterlight/levels.js','assets/afterlight/style.css'].map(read).join('\n'));
...
if(data.plays.at(-1).id!=='bamti-escape')throw new Error('The expected last existing activity changed. Review ordering before publication.');
data.plays.push(entry);
...
console.log('Registered afterlight after bamti-escape; '+data.plays.length+' activities. Version '+version);
