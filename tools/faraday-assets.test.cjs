...
const root = path.resolve(__dirname, ".."),
  atlas = require("../assets/faraday-flight/sprites.json");
test("Faraday artwork has distinct, bounded animation cels with real transparency", async () => {
  for (const [name, frames] of Object.entries(atlas)) {
    ...
    for (let i = 3; i < data.length; i += 4) {
      ...
      if (data[i] > 180) visible++;
    }
    assert.ok(
      transparent > info.width * info.height * 0.1,
      name + " needs transparent margins",
    );
    assert.ok(
      visible > info.width * info.height * 0.08,
      name + " must contain artwork",
    );
    const hashes = [];
    for (const f of frames) {
      ...
      assert.ok(
        f.ax >= f.x && f.ax <= f.x + f.w && f.ay >= f.y && f.ay <= f.y + f.h,
    ...
    assert.equal(
      new Set(hashes).size,
      frames.length,
      name + " must have genuinely distinct frames",
    );
  }
  assert.equal(atlas.hero.length, 8);
  assert.equal(atlas.enemies.length, 12);
  for (const boss of ["poverty", "gate", "symbols", "boss", "fog"])
    assert.ok(atlas[boss].length >= 4);
});
test("Faraday is immediately before Afterlight in both catalogs, with five historical stages", () => {
  ...
  require("node:vm").runInNewContext(
    fs.readFileSync(path.join(root, "assets/js/experiments-data.js"), "utf8"),
    { window: browser },
  );
  ...
  assert.equal(new Set(levels.map((l) => l.bossArt)).size, 5);
  assert.equal(new Set(levels.map((l) => l.enemyArt)).size, 5);
  for (const l of levels) {
    assert.ok(l.boss && l.story && l.source.startsWith("https://"));
    assert.equal(l.chapter.length, 3);
    assert.equal(atlas[l.enemyArt].length, 12);
  }
});
