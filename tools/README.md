...
`plays/afterlight.html`과 `assets/afterlight/`가 게임입니다. 소스 수정 후 `node tools/afterlight-register.cjs`를 실행하면 게임 내부 파일과 두 등록 목록의 캐시 버전을 함께 갱신합니다. 현재 등록 순서는 밤티 방탈출 다음 마지막입니다. 이후 다른 놀이가 추가되어 순서가 달라졌다면 이 등록 도구는 중단하므로 새 순서를 먼저 검토하세요.
